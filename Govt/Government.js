...
var Gov=Gov||{contact_list:[],email_list:[],scripts_loaded:{},scripts_total:{},area_code:"",
	...
	"Chief of Police|Chief|Clerk|Code Enforcement|Commissioner|Constable|Coordinator|Counselor|Council Member|"+
	...
	"Undersheriff)($|[^A-Za-z0-9]{1})","i"),
	      title_prefix_regex:/^(Director|Mayor|Grade|Commissioner|Senator|Police Chief|Chief|Chief [Oo]f Police|Chief Deputy|Constable|Councilman|Councilwoman|Secretary|Sheriff|Sergeant|Patrol Officer|Lieutenant|Detective|Sgt\.|Lt\.)((?:\s+[A-Z0-9\"][\-\.\'a-z0-9]+[0-9\-\.\'\"a-zA-Z]*)+)(.*)$/,
	      ...
	      bad_link_regex:/(^\s*(javascript|mailto|tel):)|(\/(cdn-cgi|tag|event|events)\/)|(\/#email)|(#$)|(\/login)|(\/events)|(-schedule(-|\/))|(\.pdf$)|(blogger\.com\/(share-post|email-post))/i,
	      ...
	      contact_regex:/Contact|Email|Directory|(^About)|^(Who We Are)|(^Departments)|Staff|Officials|^(Town|City) Hall\s*$|(^Government)|(Our Team)|(Leadership)|(Personnel)|(Our People)|((^|[^A-Za-z]+)Roster)/i};
...
Gov.guess_title_map=function(row) {
    ...
    var title_map={"department":/(Department|Dept)/i,"first":/First/i,"last":/Last/i,name:/(Staff|(Name)|Employee|Contact)/i,"email":/mail/i,
		   "title":/(Title|Position|Profession|Rank|Job|Role)/i,phone:/(Tel|Phone|Number|Voicemail)/i,"officer":/officer/i};
...
Gov.parsed_ret_is_bad=function(ret) {
    if(Gov.debug) console.log("Gov.parsed_ret_is_bad,ret="+JSON.stringify(ret));
...
// Split off a title from the beginning, may be problematic
	...
	else if(/,\s*(Ext\.)/i.test(split_lines[i])) {
	    let begin_part=split_lines[i].replace(/,\s*(Ext\.).*$/,"");
...
Gov.parse_data_func=function(text) {
   ...
   // if(Gov.debug) console.log("After skipping, j="+j);
    ...
    if(ret.email) ret.email=ret.email.replace(/^it\./,"");
