...
DQ.parse_carsforsale=function(doc,url,resolve,reject) {
    ...
    console.log("cars="+(cars?cars.innerText:""));
    ...
    if((crumb=doc.getElementsByClassName("inventory-breadcrumb")).length>0 &&
       (num=crumb[0].getElementsByClassName("section-title")[0].innerText.match(/^[\d,]+/))) {
        ...
        return; }
    if(!DQ.try_count) {
        var ret=GM_openInTab(url);
        console.log("Doing try_count, ret="+JSON.stringify(ret));
        DQ.try_count=1;
        DQ.carSearchShit(DQ.param1,DQ.param2,DQ.param3,doc,url);
        setTimeout(function() {
            ret.close();
            var promise=MTurkScript.prototype.create_promise(url,DQ.parse_carsforsale,scrape_then,my_catch_func); },3000);
        return;
    }
...
DQ.parse_v12software=function(doc,url,resolve,reject) {
    ...
    else if(link.length===1 || (link.length===0 && (pg=doc.getElementsByClassName("pagination")) && pg.length>0 && pg[0].children.length===1)) {
...
DQ.find_links_none=function(doc,url,resolve,reject,page_type) {
    var href_rx=/\/inventory/i,text_rx=/Inventory|(^Vehicles)|Listings/i,i,links=doc.links;
    var bad_href_rx=/\/posts\//i;
    var url_list=[];
    for(i=0; i < links.length; i++) {
        links[i].href=MTurkScript.prototype.fix_remote_url(links[i].href,url).replace(/\/$/,"");
        if((href_rx.test(links[i].href) || text_rx.test(links[i].innerText.trim()))&&
           !bad_href_rx.test(links[i].href)&&!url_list.includes(links[i].href)) url_list.push(links[i].href);
    }
    return url_list;
};
...
DQ["assets-cdk"]={suffix:"/VehicleSearchResults?search=used",parser:DQ.parse_assets_cdk,team_find_link:DQ.find_link,team_href_rx:/.*/,team_text_rx:/Staff|Team|Equipe|Personnel/i,team_parser:DQ.parse_team_assets_cdk};
...
DQ.convertus={team_find_link:DQ.find_link,team_href_rx:/.*/,team_text_rx:/Staff|Team|Equipe/i,team_bad_text_rx:/(^\s*Join)|Finance|Parts|Service/i,team_parser:DQ.parse_team_convertus};
DQ.d2cmedia={team_find_link:DQ.find_link,team_href_rx:/.*/,team_text_rx:/Staff|Team|Equipe|Personnel/i,team_bad_text_rx:/(^\s*Join)|Finance|Parts|Service/i,team_parser:DQ.parse_team_d2cmedia};
DQ.dealer={suffix:"/used-inventory/index.htm",parser:DQ.parse_dealer,team_find_link:DQ.find_link,team_href_rx:/.*/,team_text_rx:/Staff|Team|Equipe/i,team_bad_text_rx:/(^\s*Join)|Body Shop|Collision|Finance|Parts|Service/i,
...
DQ.dealerfire={find_link:DQ.find_link,parser:DQ.parse_dealerfire,href_rx:/\/(preowned|used)-cars/,text_rx:/View all|Inventory/i,
	      team_find_link:DQ.find_link,team_href_rx:/.*/,team_text_rx:/Staff|Team|Equipe|Personnel/i,team_parser:DQ.parse_team_dealerfire};
DQ.dealerinspire={suffix:"/used-vehicles/",parser:DQ.parse_dealerinspire,team_find_link:DQ.find_link,team_href_rx:/.*/,team_text_rx:/Staff|Team|Equipe|Personnel/i,team_parser:DQ.parse_team_dealerinspire};
DQ.dealeron={suffix:"/searchused.aspx",parser:DQ.parse_dealeron,team_find_link:DQ.find_link,team_href_rx:/.*/,team_text_rx:/Staff|Team|Equipe|Personnel/i,team_bad_text_rx:/(^\s*Join)|Finance|Parts|Service/i,
...
DQ.dealerspike={suffix:"/default.asp?page=xPreOwnedInventory",parser:DQ.parse_dealerspike,team_find_link:DQ.find_link,
