...
(function() {
    ...
    var MTurk=new MTurkScript(30000,200,[],begin_script,"A39FDYCEJJI6JQ");
        ...
        if(is_done && MTurk.doneQueries>=MTurk.queryList.length&&
           !my_query.submitted && (my_query.submitted=true) && (MTurk.submitted=true)) {
            if(my_query.fields.email.length>0) {
                if(my_query.fields.firstname.length===0) document.getElementById("firstname").value="none";
...
//        console.log(nlp_temp);
    ...
    function init_Query()
    {
        ...
        my_query={name:wT.rows[0].cells[1].innerText.replace(/\s*\(Corporate.*$/,""),try_count:{url:0,fb:0},
                  email_list:[],url:wT.rows[1].cells[1].innerText,
                  url:wT.rows[1].cells[1].innerText.replace(/^(https?:\/\/[^\s\/]*).*$/,"$1"),
        ...
        my_query.address=wT.rows[1].cells[1].innerText.replace(/^http[^\s]*/,"")+", "+
            wT.rows[2].cells[1].innerText+" "+wT.rows[3].cells[1].innerText;
        my_query.parsed_add=parseAddress.parseLocation(my_query.address);
        console.log("my_query.parsed_add="+JSON.stringify(my_query.parsed_add));
        if(my_query.parsed_add&&my_query.parsed_add.city) my_query.city=my_query.parsed_add.city;
        else my_query.city=wT.rows[2].cells[1].innerText.match(/[A-Za-z]*$/)[0];
        if(my_query.parsed_add&&my_query.parsed_add.state) my_query.state=my_query.parsed_add.state;
        else my_query.state=wT.rows[3].cells[1].innerText;

        if(my_query.city==="unknown") my_query.city="";
        if(my_query.state==="unknown") my_query.state="";
        if(/http/.test(my_query.url)) call_contact_page(my_query.url,done_url2);
        begin_searching();
        document.getElementById("firstname").addEventListener("paste",paste_name);
