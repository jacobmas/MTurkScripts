...
(function() {
    ...
    var my_query = {};
    var bad_urls=[];
    var MTurk=new MTurkScript(20000,1250+(Math.random()*300)+(Math.random()*300),[],begin_script,"A1XN77PKGB8LWI",true);
    ...
    function query_response(response,resolve,reject,type) {
        ...
        try
        {
            ...
            if(/^query$/.test(type) && b_context&&(parsed_context=MTP.parse_b_context(b_context))) {
                ...
                else if((parsed_context.SubTitle&&!/Restaurant|Dessert/i.test(parsed_context.SubTitle))||parsed_context["Hotel class"]||
                       parsed_context["School type"]!==undefined||/Golf\s.*Club/i.test(parsed_context.Title)) {
                    my_query.fields.openyesno="N/A";
                    my_query.fields.restaurantyesno="No";
                    my_query.submit_on_no=true;
                }
                else if(parsed_context.Phone) {
                    console.log("empty parse");
                   empty_parsed=true;
                }
                if(parsed_context.closed) {
                    if(my_query.fields.restaurantyesno==="") my_query.fields.restaurantyesno="Yes";
                    my_query.fields.openyesno="No";
                }
            ...
            for(i=0; i < b_algo.length&&i<4; i++) {
                ...
                if(/^query$/.test(type) && /\.(allmenus|menuism|menupix|bakerias)\.com/.test(b_url)) found_menu=true;
            ...
            if(/^query$/.test(type) && b1_success && (resolve({website:b_url,found_menu:found_menu})||true)) return;
    ...
    function query_promise_then(result) {
        ...
        if(my_query.fields.openyesno==="No"||my_query.fields.restaurantyesno==="No") {
            ...
            submit_if_done("query_promise_then");
            return;
        }
...
//            my_query.done.yelp=true;
        ...
        //console.log("Calling submit_if_done from query_promise_then");
...
//        submit_if_done();
    ...
    function yelp_promise_then(result) {
        ...
        do_yelp();
        console.log("Calling submit_if_done from yelp_promise_then");
        submit_if_done();
    ...
    function do_yelp() {
        ...
        if(document.querySelector("[name='alcohol']")) {
            ...
            console.log("NO YELP QUERY NEEDED");
            my_query.done.yelp=true;
            console.log("Calling submit_if_done from do_yelp");
    ...
    function parse_yelp_then(result) {
        ...
        my_query.done.yelp=true;
        console.log("Calling submit_if_done from parse_yelp_then");
	...
	console.log("my_query="+JSON.stringify(my_query));
        if(/^DOD([\s-]+)/.test(my_query.name)) {
            ...
            my_query.done.query=true;
            my_query.fields.restaurantyesno="No";
            console.log("Calling submit_if_done from init_query");
