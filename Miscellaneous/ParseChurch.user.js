...
(function() {
    ...
    var bad_urls=["rchurch.com","facebook.com","thecatholicdirectory.com","parishesonline.com","tripadvisor.com","local.yahoo.com","faithstreet.com"];
     var my_query = {};
    ...
    function find_links(doc,url)
    {
        ...
        var contact_regex=/(^\s*Staff)|Contact/i;
        var mass_regex=/^\s*Mass/i;
        ...
        for(i=0; i < links.length; i++) {
         ...
         //   console.log("links["+i+"].innerText="+links[i].innerText);
            if((mass_regex.test(links[i].innerText.trim()) ||
                ...
                && !my_query.mass_links.includes(links[i].href)) my_query.mass_links.push(links[i].href);
            if(contact_regex.test(links[i].innerText) && !my_query.contact_links.includes(links[i].href)) my_query.contact_links.push(links[i].href);
    ...
    function cleanup_mass() {
        var text=my_query.fields.content;
        text=text.replace(/\t|\s{2,}/g,"\n").replace(/(\n)(\t|\s)*/g,"$1");
        text=text.replace(/(?:^|\n)\s*(?:Weekday(?:s)|Weekend)[A-Za-z\s]*\n([A-Z])/ig,"$1");
    ...
    function contact_response(response) {
        var doc = new DOMParser()
        .parseFromString(response.responseText, "text/html");
        var i,j, email_val, my_match,temp_email,encoded_match,match_split;
        ...
        if(email_matches) for(i=0; i < email_matches.length; i++) if(!MTurk.is_bad_email(email_matches[i]) && ( my_query.fields.email=email_matches[i])) break;
        for(i=0; i < links.length; i++) {
            ...
            if(links[i].dataset.encEmail && (temp_email=MTP.swrot13(links[i].dataset.encEmail.replace(/\[at\]/,"@")))
               && !MTP.is_bad_email(temp_email)) my_query.fields.email=temp_email;
            ...
            if(links[i].href.indexOf("javascript:DeCryptX(")!==-1 &&
               (encoded_match=links[i].href.match(/DeCryptX\(\'([^\)]+)\'\)/))) my_query.fields.email=MTP.DecryptX(encoded_match[1]);
    ...
    /* Facebook search stuff fuck I did better than this now */
...
//        not sure if this is consistently going to be enough to identify it tho
    ...
    function add_to_sheet() { for(var x in my_query.fields) document.getElementById(x).value=my_query.fields[x]; }

    function paste_mass(e) {
        e.preventDefault();
    ...
    function init_Query()
    {
        ...
        document.getElementById("content").addEventListener("paste",paste_mass);
        for(x in my_query.fields) if(x!=='content') document.getElementById(x).addEventListener("paste",paste_other);
