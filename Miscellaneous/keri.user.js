...
// @version      0.1
// @description  Find Job Title, location given name, company
// @author       You
...
(function() {
    ...
    var my_query = {};
    var bad_urls=[];
    var MTurk=new MTurkScript(20000,750+(Math.random()*750),[],begin_script,"A22CHW0LJ94ZKK",true);
    ...
    function query_response_loop(b_name,b_url,p_caption,type,promise_list,i) {
        ...
        if(short_name.toLowerCase().indexOf(my_query.fullname.lname.toLowerCase())!==-1) {
        ...
        if((/query/.test(type)) && /linkedin\.com\/in/.test(b_url) && full_short.fname===my_query.fullname.fname &&
           full_short.lname===my_query.fullname.lname) {
            ...
            if(match) {
                Object.assign(my_query.fields,{url:b_url,website:b_url,"Job Title":match[2].trim(),company:match[3].trim()});
                return true;

            }
        }
        console.log("BUMP "+JSON.stringify(my_query.fullname));
        ...
        if(/bloomberg\.com\/profiles\/people/.test(b_url)) {
            ...
            if((match=p_caption.match(bloom_reg))) {
                ...
                my_query.fields["Job Title"]=match[1].trim();
        ...
        if(/bizapedia\.com/.test(b_url)) {
            ...
            if((match=p_caption.match(bizapedia_re))) {
                Object.assign(my_query.fields,{url:b_url,"Job Title":match[1].trim(),company:match[2].trim()});
                return true;
            }
        }
        ...
        if(/psychologytoday\.com/.test(b_url) ) {
            ...
            if(split.length>=2) {
                Object.assign(my_query.fields,{url:b_url,"Job Title":split[1].trim(),company:"Self-Employed"});
                return false;
            }
        }
        ...
        for(curr_person of people) {
            ...
            if(people.length<2 || curr_person.location.length===0 ||
               ...
               curr_person.location.indexOf(my_query.city)!==-1) {
                ...
                let x;
                var is_bad=false;
                let temp_regexp=new RegExp(my_query.fields.Company,"i");
                if(!temp_regexp.test(curr_person.title)) {
                    console.log("temp_regexp failed, temp_regexp="+temp_regexp);
                    for(x in state_map) {
                        //console.log("x="+x);
    ...
    function query_response(doc,url,resolve,reject,type) {
        ...
        try
        {
            ...
            if(b_context&&(parsed_context=MTP.parse_b_context(b_context))) {
                ...
                if(parsed_context.LinkedIn) {
                    my_query.fields.website=parsed_context.LinkedIn;
                    add_to_sheet();
                }
                ...
                if(parsed_context.person &&
                   ((parsed_context.person.experience&&parsed_context.person.experience.length>0)||parsed_context.person.Current)) {
                    if(handle_person(parsed_context.person)) {
                        resolve("");
                        return true;
                    }
                }
    ...
    function handle_person(person) {
        console.log("In handle_person,person="+JSON.stringify(person));
        var x;
        if(person.linkedin_url) my_query.fields.linkedin_url=person.linkedin_url;

        return true;

    }

    /* handle a parsed person */
    function handle_person_old(person) {
        var match;
        if(!my_query.fields["First Name"]) {
            let fullname=MTP.parse_name(person.name);
            Object.assign(my_query.fields,{"First Name":fullname.fname,"Last Name":fullname.lname});
        }
        my_query.fields.url=person.linkedin_url;
        if(person.Location) {
            if(!/,/.test(person.Location)) {
                my_query.fields.Country="United States";
                my_query.fields.City=person.Location;
            }
            else {
                let split=person.Location.split(/,\s*/);
                if(split.length===2 && state_map[split[1].trim()]!==undefined || reverse_state_map[split[1].trim()]!==undefined) {
                    my_query.fields.Country="United States";
                    my_query.fields.City=split[0];
                }
                else {
                    my_query.fields.City=split[0].trim();
                    my_query.fields.Country=split[split.length-1].trim();
                }
            }
        }
        if(person.experience&&person.experience.length>0) {

            my_query.fields["Job Title"]=person.experience[0].title;

        }
        else {
            let split=person.Current.split(/,\s*/);
            my_query.fields["Job Title"]=split[0];
            if(split.length>1) my_query.fields.company=split[1];
            else {
                match=person.Current.match(/^(.*?) at (.*)$/);
                if(match) {
                    my_query.fields["Job Title"]=match[1];
        ...
        if(fullname.fname.charAt(0)===my_query.fullname.fname.charAt(0) ||
           MTP.longest_common_subsequence(my_query.fullname.fname,fullname.fname).length>=2) {
            ...
            my_query.fields["Job Title"]=my_query.fields.company=my_query.fields.url="";
        }
        return false;
    }
...
.parseFromString(response.responseText, "text/html");
    ...
    function add_to_sheet() {
        var x,field,i;
        var field_name_list=["linkedin_url","consulting","tech"];
        var fields=document.querySelectorAll("input.form-control");
        for(i=0;i<fields.length;i++) {
            fields[i].value=my_query.fields[field_name_list[i]];
    ...
    function submit_if_done() {
        ...
        if(is_done && !my_query.submitted && (my_query.submitted=true)) {
            if(my_query.fields.City.length>0&&my_query.fields.Country.length>0 && my_query.fields["Job Title"].length>0) MTurk.check_and_submit();
            else {
    ...
    function init_Query() {
        ...
        my_query={name:wT.rows[0].cells[1].innerText.trim(),company:"",
                  city:"",state:"",country:"",fields:{"linkedin_url":"",tech:"N/A",consulting:"N/A"},synonyms:[],synonym_pos:0,
                  done:{query:false,redone_query:false},
                  submitted:false,redone_linkedin:false,
                 try_count:{"query":0}};

	console.log("my_query="+JSON.stringify(my_query));
        if(my_query.name.trim().length>0) {
            let fullname=MTP.parse_name(my_query.name.trim());
            my_query.fullname=fullname;
        }

        var search_str=
           my_query.name+" consultant site:linkedin.com";
