...
(function() {
    ...
    var my_query = {};
    var bad_urls=[];
    var MTurk=new MTurkScript(40000,1000,[],begin_script,"AZ1YMGWH50JEU",true);
    ...
    School.prototype.search_none=function(doc,url,resolve,reject,extra) {
        ...
        if(depth>0) good_link_str="(^(Faculty|Admin|District|Central|Personnel|Employee|Contact|Directory|Staff|About|Leadership|Team|Departments))";
        var good_link_re=new RegExp(good_link_str,"i");
        var i,links=doc.links,promise_list=[];
        Gov.query={};
        Gov.query.title_regex_lst=self.title_regex;
...
//            console.log("Email for "+i+"="+results[i].email);
    ...
    function submit_if_done() {
        ...
        if(is_done && !my_query.submitted && (my_query.submitted=true)) {
            if(my_query.fields.email.length>0&&
               !(my_query.fields.email==="none@none.com" && my_query.fields.first_name !== "none")) MTurk.check_and_submit();
            else {
    ...
    function PersonQual(curr) {
        ...
        this.quality=0;
        if(curr.title && /Principal|Dean|Head of School|Director|Administrator/i.test(curr.title)) {
            this.type="Administration";
            if(/^(Principal|Head of School)$/i.test(curr.title)) this.quality=8;
                   else if(/(Principal|Head of School)/i.test(curr.title)) this.quality=8;

            else if(/^((Co-)?Director|Dean)/i.test(curr.title)) this.quality=2;
            else this.quality=1;
        }
        if(/[\d\?]+/.test(this.name)) this.quality=-1;
        var nlp_out=nlp(this.name).people().out('topk');
        if(this.email&&this.email!=="na") this.quality+=6;
        if(this.email==="na") this.email="none@none.com";
    ...
    function init_Query() {
       ...
       var crowd=document.querySelectorAll("crowd-input");
        crowd.forEach(function(elem) { elem.required=false; });
        my_query={url:document.querySelectorAll("form a")[1].href,fields:{first_name:"",last_name:"",email:""},
                  done:{},submitted:false};
        my_query.query=query;
        console.log("my_query="+JSON.stringify(my_query));
var s;
        if(/mturkcontent\.com/.test(my_query.url)) {
             my_query.fields.first_name="none";
            my_query.fields.last_name="none";
            my_query.fields.email="none@none.com";
        }

        var promise=new Promise((resolve,reject) => {
            s=new School({url:my_query.url,type:"school",
                              title_str:["Principal","Head of School","Director","Dean"],
                              debug:true,
                             title_regex:[/Principal|Director|Head of School|Dean|Administrator/i]},resolve,reject);
        ...
        promise.then(function(self) {
            ...
            if(result.length>0) {
                ...
                if(curr.email!=="none@none.com"||Gov.email_list.length===0) {
                    my_query.fields.email=curr.email; }
                else my_query.fields.email=Gov.email_list[0];

                
            }
            else if(Gov.email_list.length>0) {
                my_query.fields.email=Gov.email_list[0];
                my_query.fields.first_name=my_query.fields.last_name="none";
            }
