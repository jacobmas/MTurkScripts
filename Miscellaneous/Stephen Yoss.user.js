...
(function() {
    ...
    var MTurk=new MTurkScript(20000,750+(Math.random()*1000),[],begin_script,"A33XNP9D6W54VC",false);
    ...
    if(/mturkcontent\.com/.test(window.location.href))     begin_script(1000);
    ...
    function init_Query()
    {
        ...
        var record=document.querySelector(".contact-record").innerText;
        record=record.split("\n");
        ...
        my_query={name,fields:{},done:{},
		  ...
		  submitted:false};
        var line;
        var term_map={"Last Name":"lname","First Name":"fname", "Middle Name":"minit","DBA":"company",
                      "Address Line #1":"address1","City":"city","State":"state","Post Code":"zip"};
        for(line of record) {
            let curr=line.split(/\s\-\-\s/);
            if(curr.length<2) continue;
            console.log("curr=",curr);
            if(term_map[curr[0]]!==undefined) {
                my_query[term_map[curr[0]]]=curr[1];
