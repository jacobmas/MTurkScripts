...
(function() {
    ...
    var MTurk=new MTurkScript(20000,750+(Math.random()*1000),[],begin_script,"A3V327EQ80OXN3",true);
    ...
    function parse_amazon(doc,url,resolve,reject) {
        ...
        var avail=doc.querySelector("#availability-string");
        console.log("avail=",avail);
        var price=doc.querySelector("#priceblock_ourprice");
        if(!avail) {
            my_query.fields.price="NA";
            resolve("");
            return;
        }
        let availText=avail.innerText.trim();
        console.log("availText=",availText);
        console.log("price=",price);
        if(/\s*In Stock\./.test(availText)) {
            console.log("In stock");
            let checkbox=document.querySelector("crowd-checkbox");
            console.log("checkbox=",checkbox);

            checkbox.click();
        }
        my_query.fields.price=price.innerText.replace(/\$/,"").trim();
        resolve("");
    }
	...
	console.log("my_query="+JSON.stringify(my_query));
       var promise=MTP.create_promise(url,parse_amazon,submit_if_done, function() { GM_setValue("returnHit",true); });
    }
