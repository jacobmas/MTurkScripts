...
(function() {
    ...
    var Gov=Gov||{contact_list:[],scripts_loaded:{},scripts_total:{},area_code:"",
                 ...
                 title_regex:new RegExp("(^|[\\s,\\.]{1})(Director|Department|Supervisor|Manager|Clerk|Administrator|"+
                                        "Council Member|Attorney|Recorder|Official|Coordinator|Mayor|Planner|Engineer|Police|"+
                                        "Superintendent|"+
                                        "Chief)($|[\\n\\s,\\. ]{1}|[^A-Za-z0-9]{1})","i")};
   ...
   Gov.parse_municodeweb_no_directory=function(result) { };
    ...
    Gov.scrape_none=function(doc,url,resolve,reject) {
        ...
        for(i=0; i < Gov.dept_links.length; i++)
        {
            ...
            promise_list.push(MTurkScript.prototype.create_promise(
                Gov.dept_links[i].url,Gov.parse_department,Gov.parse_contacts_then,MTurkScript.prototype.my_catch_func,Gov.dept_links[i].name.trim()));
        }
    ...
    **/
    Gov.parse_department=function(doc,url,resolve,reject,name) {
        var add_count;
     ...
     /*   for(i=0; i < Gov.depts[name].contact_links.length; i++)
        {
            ...
            // First load the scripts and tables into a non-displayed temp_div (there will be several), this should hopefully unhide the emails
              //  though you may need to load other scripts too, then parse them
          //  promise_list.push(MTurkScript.prototype.create_promise(
            //    Gov.depts[name].contact_links[i],Gov.load_scripts,Gov.parse_contacts_then));
        } */
    ...
    /* Gov.parse_contact_div, may require splitting into multiple contacts */
    Gov.parse_contact_div=function(elem,name) {
        var ret,p_adds=0,add_count=0;
        var bolds=elem.querySelectorAll("b"),i;

        if(elem.querySelector("div") && elem.querySelector("div").querySelector("div")) return 0;
        for(i=0;i<bolds.length; i++) { console.log("bolds["+i+"]="+bolds[i].innerText); }
        Gov.fix_emails(elem);
        if((ret=Gov.parse_data_func(elem.innerText)) && ret.name && ret.title && ret.email
    ...
    Gov.strip_bad_contacts=function() {
        ...
        for(i=Gov.contact_list.length-1; i>=0; i--) {
            ...
            if(phone_re.test(Gov.contact_list[i].name) || temp_list.includes(temp_push) ||
              Gov.contact_list[i].name.length>60 || Gov.contact_list[i].title.length>60) Gov.contact_list.splice(i,1);
    ...
    Gov.fix_emails=function(div,is_civic) {
        var inner_a=div.querySelectorAll("a"),cf_email,result;
        inner_a.forEach(function(elem) {
            if(/^\s*mailto:\s*/.test(elem.href)) elem.innerHTML=elem.innerHTML+"\t"+elem.href.replace(/^\s*mailto:\s*/,"").replace("%20","")+"\t";
    ...
    Gov.parse_table=function(table,title_map,begin_row,end_row,dept) {
        ...
        for(i=begin_row; i < end_row; i++)  {
            ...
            for(j=0; j < curr_row.cells.length; j++)
            {
                ...
                if(title==="email" && (inner_a=curr_cell.getElementsByTagName("a")).length>0) {
                    curr_contact[title]=inner_a[0].href.replace(/\s*mailto:\s*/,"").replace(/\?.*$/,"").trim().replace(/\s+\n\s+/g," ").replace(/\n.*$/,"");
                }
                else if(title==="email") {
                ...
                if(title_map.email===undefined && (inner_a=curr_cell.getElementsByTagName("a")).length>0 &&
                  /^\s*mailto:\s*/.test(inner_a[0].href)) curr_contact.email=inner_a[0].href.replace(/^\s*mailto:\s*/,"").replace(/\n.*$/,"");
            }
            ...
            for(i=0;i<row.cells.length; i++)
            {
                ...
                if(email_re.test(curr_cell) ||
                   ((inner_a=row.cells[i].getElementsByTagName("a")).length>0 && /^\s*mailto:\s*/.test(inner_a[0].href))) ret.email=i;
                        ...
                        !(ret.title===undefined &&
                          Gov.title_regex.test(curr_cell))) ret.name=i;
    ...
    Gov.get_department_contact_links=function(doc,url,resolve,reject,dept) {
        ...
        Gov.depts[dept].contact_links=[];
         var i,j;
        var contact_regex=/Contact|Staff/i,bad_link_regex=/(^\s*(javascript|mailto):)|(\/tag\/)/i;
    ...
    */
    Gov.get_contact_links=function(doc,url,resolve,reject) {
        var i,j;
        var contact_regex=/Contact|Email|Directory|(^About)|(^Departments)|Staff|Officials/i,bad_link_regex=/(^\s*(javascript|mailto):)|(\/tag\/)/i;
        var contacthref_regex=/contact/i;
        Gov.contact_links=[];
        var out_str="";
        for(i=0; i < doc.links.length; i++) {
