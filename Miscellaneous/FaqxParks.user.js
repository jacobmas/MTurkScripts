...
(function() {
    ...
    function query_response(response,resolve,reject,type) {
	    ...
	    if(b_context&&(parsed_context=MTP.parse_b_context(b_context))) {
		   ...
		   && (b1_success=true)) break;
                if(type=="fb" && !MTP.is_bad_fb(b_url) && !MTurkScript.prototype.is_bad_name(b_name,my_query.name.replace(/,.*$/,""),p_caption,i) && !/\/places\//.test(b_url)
		   && (b1_success=true)) break;
    ...
    function query_promise_then(result) {
        ...
        var promise=MTP.create_promise(result,parse_website,parse_website_then, function() { my_query.done.query=true; submit_if_done(); });
    ...
    function parse_website(doc,url,resolve,reject) {
        var phone=Gov.find_phone(doc,url);

        if(phone&&!my_query.fields.phone_general) {
            my_query.fields.phone_general=phone;
        }
        else {
            phone=doc.body.innerText.match(phone_re);

            if(phone&&!my_query.fields.phone_general) my_query.fields.phone_general=phone[0].trim();
        }
        resolve("");
    }
