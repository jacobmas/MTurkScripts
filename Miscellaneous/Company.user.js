...
(function() {
    ...
    var MTurk=new MTurkScript(20000,750+(Math.random()*1000),[],begin_script,"A1CO31FO1H7RUE",true);
    ...
    function query_promise_then(result) {
        ...
        var promise=MTP.create_promise(result,find_logo,submit_if_done,function() { GM_setValue("returnHit",true); });
    }

    function find_logo(doc,url,resolve,reject) {
         var logo=doc.querySelector("img[id*='logo' i],img[src*='logo' i],img[data-src*='logo' i");
        if(logo) {
            my_query.fields.logo=logo.src;
            resolve("");
            return;
        }
        reject("");
    ...
    function init_Query()
    {
        ...
        var strong=document.querySelector("crowd-form strong").innerText.trim();
        var match=strong.match(/(.*) based in (.*)/);

        my_query={name:match[1],country:match[2],fields:{},done:{},
		  try_count:{"query":0},
		  submitted:false};
