// ==UserScript==
// @name         Roc HargroveFAX
// @namespace    http://tampermonkey.net/
// @version      0.1
// @description Copy of DoctorDB
// @author       You
...
(function() {
    'use strict';
    var my_query = {};
    var bad_urls=['.addresses.com','/allpeople.com','.angmedicare.com','/arrestfacts.com','.arounddeal.com','.bbb.org',
    ...
    var MTurk=new MTurkScript(30000,1000,[],begin_script,"A1SK2GV23YJWN9",true);
   ...
   function scrape_ascension(doc, url, locations) {
    ...
    function scrape_doctor(doc,url,resolve,reject,extra) {
     //   console.log("scrape_doctor, doc=",doc.body.innerHTML);
        ...
        if(phone)  my_query.fields['07_phone_01']=phone.innerText.trim().replace(/[^\d]+/g,"").replace(/^1/,"");
        if(!phone) {
            let temp_phone=doc.querySelector("a[href^='tel']");
            console.log("phone=",temp_phone);
            if(temp_phone) {
                phone=temp_phone.href.trim().replace(/[^\d]+/g,"").replace(/^1/,"");
                if(!/^0{3,}/.test(phone)&&phone.length>=10) my_query.fields['07_phone_01']=phone;
                else phone="";
            }
        }
        if(fax) my_query.fields['08_fax_01']=1+fax.innerText.trim().replace(/[^\d]+/g,"").replace(/^1/,"");;
        if((my_query.fields['07_phone_01']||(phone=doc.body.innerText.match(/(?:Tel|Phone):\s*\n*\s*([\([(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6})/im))) && !fax &&
           (fax=doc.body.innerText.match(/Fax:\s*\n*\s*([\([(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6})/im))) {
            console.log("phone=",phone,"fax=",fax);
            if(!my_query.fields['07_phone_01'])    my_query.fields['07_phone_01']=phone[1].trim().replace(/[^\d]+/g,"").replace(/^1/,"");
            else { console.log("my_query.fields['office1_phone_number']=",my_query.fields['office1_phone_number']); }
            my_query.fields['08_fax_01']=1+fax[1].trim().replace(/[^\d]+/g,"").replace(/^1/,"");
            add_to_sheet();
    ...
    function address_scrape_then(result) {
        ...
        if(Address.addressList.length>=1) {
            ...
            my_query.fields['01_office_name1_01']=result[0].name;
    ...
    function query_md_practice(doc,url,resolve,reject) {
        ...
        my_query.fields['07_phone_01']=phone.replace(/[^\d]+/g,"").replace(/^1/,"");;
        my_query.fields['08_fax_01']=1+fax.replace(/[^\d]+/g,"").replace(/^1/,"");;
    ...
    function add_to_sheet() {
        ...
        for(i=0;i<2&&i<my_query.office_list.length;i++) {
            for(x in my_query.office_list[i]) {
                let short_x=x.replace("_number","").replace("source_","").replace("name","office_name1");
                let suffix_str=i==0?"01":"02";
                let field_str1=short_x+"_"+suffix_str;
                let field_str=add_field_prefix[field_str1]+"_"+field_str1;
                console.log("field_str=",field_str);
                my_query.fields[field_str]=my_query.office_list[i][x];
            }
        }
    ...
    function update_address(address,suffix) {
        ...
        //    console.log("Adding "+x+"_"+suffix);
            let suffix_str=suffix<9 ? "01":"02"
           let field_added=add_field_prefix[x+"_"+suffix_str]+"_"+x+"_"+suffix_str;
           // console.log("Adding ",field_added);
           my_query.fields[field_added]=top[x];
        }
    ...
    function remove_phones(text,suffix,target) {
        ...
        console.log("remove_phones, suffix=",suffix);
            let suffix_str=suffix<9 ? "01":"02"
        ...
        console.log("pasted_name=",pasted_name);
        for(i=0;i<split.length;i++) {
            if(fax_match=split[i].match(/Fax:\s*([\(]?[0-9]{3}([\)]?[-\s\.\/]|\))[0-9]{3}[-\s\.\/]+[0-9]{4,6})/)) {
                my_query.fields[fax_prefix_map[suffix_str]+"_fax_"+suffix_str]="1"+fax_match[1].replace(/[^\d]+/g,"");
            ...
            if(i===0 && pasted_name && !/^.{0,10}[\d]/.test(split[i])) {
                ...
                let field_str=(suffix<9?"01":"10")+"_office_name1_"+suffix_str;
                console.log("field_str=",field_str);
                my_query.fields[field_str]=split[i].trim();
                continue;
            }
            match=split[i].match(phone_re);
            if(match) {
                if(!matched_phone && (matched_phone=true)) {
                    my_query.fields[phone_prefix_map[suffix_str]+"_phone_"+suffix_str]=match[0].replace(/[^\d]+/g,"");
                }
                else {
                    my_query.fields[fax_prefix_map[suffix_str]+"_fax_"+suffix_str]="1"+match[0].replace(/[^\d]+/g,""); }
            }
            else {
    ...
    function do_address_paste(e) {
        ...
        var match=e.target.name.match(/01$/);

        var suffix=match?1:10;
    ...
    function parse_healthgrades(doc,url,resolve,reject) {
      ...
      //  console.log("loc=",loc);
        ...
        if(fax) {
            let y;
            for(y of h3) {
                if(y && !/^\s*Practice\s*$/.test(y.innerText.trim())) {  my_query.fields["01_office_name1_01"]=y.innerText.trim(); break; }
        ...
        //    console.log("add=",add);
            ...
            if(phone) my_query.fields["07_phone_01"]=phone.href.replace(/[^\d]+/g,"").replace(/^1/,"");
            if(fax) my_query.fields["08_fax_01"]="1"+fax.href.replace(/[^\d]+/g,"").replace(/^1/,"");
            //console.log("phone=",phone,"fax=",fax);
            my_query.fields["09_website_01"]=url;
        }
    ...
    function init_Query()
    {
      ...
      //var end_re=/[A-Z][a-z]+(\s)
        ...
        var phone_list=["07_phone_01","16_phone_02"];
        var fax_list=["08_fax_01","17_fax_02"];
        ...
        for(x of phone_list) {
         ...
         for(x of fax_list) {
          ...
          for(x of ['09_website_01','18_website_02']) {
