// ==UserScript==
// @name         MelissaHalliburtonTripAdvisor
...
/*            if(b_context&&(parsed_context=MTP.parse_b_context(b_context))) {
                ...
                if(parsed_context.url&&!MTP.is_bad_url(parsed_context.url,bad_urls,-1)) {
                    resolve(MTP.fix_bing_link(parsed_context.url));
                    return;
                }
            }
            if(lgb_info&&(parsed_lgb=MTP.parse_lgb_info(lgb_info))) {
                ...
                if(parsed_lgb.url&&!MTP.is_bad_url(parsed_lgb.url,bad_urls,-1)) {
                    resolve(MTP.fix_big_link(parsed_lgb.url));
                    return;
                }
            ...
            for(i=0; i < b_algo.length&&i<2; i++) {
                ...
                if(!MTurkScript.prototype.is_bad_name(b_name,my_query.name,p_caption,i)
                   && (b1_success=true)) break;
            }
            if(b1_success && (resolve(MTP.fix_bing_link(b_url))||true)) return;
        }
        catch(error) {
            reject(error);
            return;
    ...
    function query_promise_then(result) {
        var match;
        if((match=result.match(/welcome\?url\=([^&]*)&/))) {
    ...
    function init_Query()
    {
        ...
        my_query={name:wT.rows[0].cells[1].innerText.trim(),address:wT.rows[1].cells[1].innerText.trim(),fields:{},done:{},
                  try_count:{"query":0},
                  submitted:false};
        console.log("my_query="+JSON.stringify(my_query));
        var search_str=my_query.name+" "+my_query.address+" site:tripadvisor.com";
