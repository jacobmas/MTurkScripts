// ==UserScript==
// @name         FindPersonalEmailSeniorPosition
...
(function() {
    ...
    var MTurk=new MTurkScript(60000,500,[],begin_script,"AR09EJ3MUIJ44",true);
    ...
    function query_response_loop(b_algo,i,type,promise_list,resolve,reject,b1_success) {
        ...
        if(type==="company" && /\/company\//.test(b_url)) {
            let name=b_name.replace(/ LinkedIn\s*$/,"").replace(/\s+([\|\-]+).*$/,"");
            return name;
        }
        if(type==="person" && /\/in\//.test(b_url)) {
            let b_split=b_name.replace(/[\-\|]\s*LinkedIn$/i,"").split(/\s+[\-\|]\s+/);
            if(b_split.length>=2) {
                my_query.name=b_split[0].trim();
                ...
                my_query.fields.title=b_split[1].trim();
                add_to_sheet();
                return "DONE";
            }
            //if(b_split.length>=3 && !/\.\.\.\s*$/.test(b_split)) my_query.company=b_split[2].replace(/\.\.\.\s*$/,"");
        ...
        try {
            ...
            if((b_context=doc.getElementById("b_context"))&&(parsed_context=MTP.parse_b_context(b_context))) {
             ...
             //   console.log("b_context="+b_context.innerHTML);
                ...
                if(!/company|person/.test(type) && parsed_context.people&&!parsed_context.person) {
                ...
                if(type==="company" && parsed_context.name && /\(Company\)/.test(parsed_context.name)) {
                    resolve(parsed_context.name.replace(/\(Company\)/,"").trim());
                    return;
                }
...
//                var promise=MTP.create_promise(curr_person.url,query_response,resolve,reject,type);
    ...
    function company_promise_then(result) {
        ...
        const queryPromise = new Promise((resolve, reject) => {
            ...
            query_search("\""+my_query.company+"\" (CEO or Manager) site:linkedin.com/in", resolve, reject, query_response,"person");
        });
        queryPromise.then(person_promise_then)
            .catch(function(val) {
    ...
    function person_promise_then(result) {
        console.log("Done person_promise_then, heading to url_promise_then");
        url_promise_then(my_query.domain);
    }
...
//            query_search(search_str, resolve, reject, query_response,"email");
    ...
    /* Kevin Bryan only */
    function parse_initial_params() {
        var url=document.querySelectorAll("form div a");
        my_query.domain=url[1].innerText;
    }
    ...
    function setup_email_guesses(the_name) {
        if(the_name===undefined) the_name=my_query.fullname;
        var lname=MTP.removeDiacritics(the_name.lname).replace(/[\'\s]/g,"").toLowerCase(),fname=MTP.removeDiacritics(the_name.fname).replace(/\'/g,"").toLowerCase();
...
function proper_casing(match,p1,p2) {
    ...
    function paste_name(e) {
        ...
        if(ret.title) my_query.fields.title=ret.title;
    ...
    function init_Query()
    {
     ...
     //   var wT=document.getElementById("DataCollection").getElementsByTagName("table")[0];
        ...
        document.querySelector("crowd-input[name='firstName']").addEventListener("paste",paste_name);
        add_to_sheet();

        const queryPromise = new Promise((resolve, reject) => {
            console.log("Beginning URL search");
            query_search(my_query.domain+" site:linkedin.com", resolve, reject, query_response,"company");
        });
        queryPromise.then(company_promise_then)
            .catch(function(val) {
            my_query.done.url=true;
            my_query.done.query=true;
            console.log("Failed at this queryPromise " + val);  submit_if_done(); });
