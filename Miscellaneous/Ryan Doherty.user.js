...
// @version      0.1
// @description  One off Find Phone
// @author       You
...
(function() {
    ...
    function query_response(response,resolve,reject,type) {
	    ...
	    if(b_context&&(parsed_context=MTP.parse_b_context(b_context))) {
            ...
            if(parsed_context.Phone) {
                my_query.fields.PhoneNumber=fix_phone(parsed_context.Phone);
                resolve("");
                return;
            }
        }
            if(lgb_info&&(parsed_lgb=MTP.parse_lgb_info(lgb_info))) {
                ...
                if(parsed_lgb.phone) {
                    my_query.fields.PhoneNumber=fix_phone(parsed_lgb.phone);
                    resolve("");
                    return;
                }
            }
            for(i=0; i < b_algo.length; i++) {
                ...
                if((b_factrow=b_algo[i].querySelector(".b_factrow")) &&
                   (parsed_factrow=MTP.parse_b_factrow(b_factrow))) {
                    if(parsed_factrow.Phone) {
                        my_query.fields.PhoneNumber=fix_phone(parsed_factrow.Phone);
                        resolve("");
                        return;
                    }
    ...
    function init_Query()
    {
        ...
        var i;
        var wT=document.querySelectorAll("form section table")[1];
      //  var a=document.querySelectorAll("form a");
        my_query={name:wT.rows[1].cells[0].innerText,address:wT.rows[2].cells[0].innerText.trim()
                  ,fields:{},
                  done:{},
		  try_count:{"query":0},
		  submitted:false};
