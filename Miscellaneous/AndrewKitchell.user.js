...
(function() {
    ...
    var MTurk=new MTurkScript(20000,750+(Math.random()*1000),[],begin_script,"AFNSQ9PNM9UPU",true);
    ...
    function query_response(response,resolve,reject,type) {
	    ...
	    if(b_context&&(parsed_context=MTP.parse_b_context(b_context))) {
            ...
            if(parsed_context.url&&!MTP.is_bad_url(parsed_context.url)) {
    ...
    function query_promise_then(result) {
        my_query.fields.Website=my_query.url=result;
      ...
      function gov_promise_then(my_result) {
        ...
        console.log("\n*** Gov.phone="+Gov.phone);
        if(!my_query.fields["Phone Number"] && Gov.phone) {
            my_query.fields["Phone Number"]=Gov.phone;
        }
        if(Gov.email_list.length>0) {
            Gov.email_list.sort(function(a,b) {
                try {
                    var a_domain=a.email.replace(/^.*@/,""),b_domain=b.email.replace(/^.*@/,"");
                    if(a_domain===my_query.domain && b_domain!=my_query.domain) {
                        return -1;
                    }
                    else if(b_domain===my_query.domain && a_domain!=my_query.domain) {
                        return 1;
                    }
                    else {
                        return 0;
        ...
        if(my_query.fields.Email && my_query.fields["Phone Number"]) {
            submit_if_done();
        }
        else {
...
//        console.log("result="+JSON.stringify(result));
    ...
    function init_Query()
    {
        ...
        var i;
        var name=document.querySelectorAll("form p")[1].innerText.trim();

        my_query={name:name,fields:{"Website":"","Phone Number":"","Email":""},done:{},
		  try_count:{"query":0},
		  submitted:false};
