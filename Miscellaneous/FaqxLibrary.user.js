...
(function() {
    ...
    function parse_library(doc,url,resolve,reject) {
        var x;
        for(x of doc.links) {
            if(/instagram\.com/.test(x.href)) my_query.fields.url_instagram=x.href;
            if(/\.youtube\.com/.test(x.href)) my_query.fields.url_youtube=x.href;
            if(/Library Card/i.test(x.innerText) && !my_query.fields["url_library-card"]) my_query.fields["url_library-card"]=x.href;
            if(/Catalog/i.test(x.innerText) && !my_query.fields["url_online-catalog"]) my_query.fields["url_online-catalog"]=x.href;
    ...
    function init_Query()
    {
        ...
        var i;
       var url=document.querySelector(".websiteUrl");
        console.log("url=",url);

        var my_url=url.onclick.toString().replace(/^[^`]*`/,"").replace(/`[^`]*$/,"");
        console.log("my_url=",my_url);
        my_query={url:my_url,fields:{"url_online-catalog":"","url_library-card":"","url_youtube":"","url_instagram":""},done:{},
		  try_count:{"query":0,"fb":0,"twitter":0},
		  submitted:false};

       var promise=MTP.create_promise(my_query.url,parse_library, submit_if_done, function() { GM_setValue("returnHit",true); })
