...
(function() {
    ...
    var MTurk=new MTurkScript(20000,750+(Math.random()*1000),[],begin_script,"A93PSCPNMK1GK",true);
    ...
    function init_Query()
    {
        // div, beverage, combo, unclassified
        document.querySelector("crowd-radio-button[name='dish']").click();
        var div=document.querySelector("div[style^='background");
        var desc1=div.querySelector("i").innerText.trim();
        var desc2=div.querySelector("b").innerText.trim();
        var desc3=div.innerText.trim();

        if(/Appetizers/i.test(desc1)) {
                    document.querySelector("crowd-radio-button[name='dish']").click();
	setTimeout(submit_if_done,1000);
            return;
        }
        if(/Beverage|coffee/i.test(desc1)) {
            document.querySelector("crowd-radio-button[name='beverage']").click();
            	setTimeout(submit_if_done,1000);
            return;
        }
        if(/Served with/i.test(desc3)||/combo|package/i.test(desc1)||/package/i.test(desc2)) {
             document.querySelector("crowd-radio-button[name='combo']").click();
            	setTimeout(submit_if_done,1000);
            return;
        }
