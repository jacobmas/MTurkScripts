...
(function() {
    ...
    var MTurk=new MTurkScript(20000,750+(Math.random()*1000),[],begin_script,"AHGUX6PWHO78L",true);
    ...
    function query_response(response,resolve,reject,type) {
	    ...
	    if(b_context&&(parsed_context=MTP.parse_b_context(b_context))) {
            ...
            if(parsed_context.person&&parsed_context.person.experience&&parsed_context.person.experience.length>0&& MTP.matches_names(my_query.name,parsed_context.person.name)) {

                resolve(parsed_context.person.linkedin_url);
                return;
            }
         if(parsed_context.people &&parsed_context.people.length>0 && my_query.try_count[type]<3) {
             ...
             var person;
                for(person of parsed_context.people) {
                    if(!/ is a /.test(person.title) && (my_query.name === person.name)) {
    ...
    function init_Query()
    {
        ...
        var i;
       var name=document.querySelector("crowd-form div div div").innerText.replace(/^[^:]*:\s*/,"");
        my_query={name,fields:{},done:{},
		  try_count:{"query":0},
		  submitted:false};
	console.log("my_query="+JSON.stringify(my_query));
        var search_str="\""+my_query.name+"\" law new york site:linkedin.com";
