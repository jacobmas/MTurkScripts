...
// @version      0.1
// @description  One Off Find Website
// @author       You
...
(function() {
    ...
    var my_query = {};
    var bad_urls=[];
    var MTurk=new MTurkScript(20000,750+(Math.random()*1000),[],begin_script,"A2CUXDPWR1XHXU",true);
    var MTP=MTurkScript.prototype;
    function is_bad_name(b_name,p_caption,i)
    {
        var reg=/[-\s\'\"’]+/g,b_replace_reg=/\s+[\-\|–]{1}.*$/;
        ...
        var b_name_split=b_name.split(b_replace_reg);
        my_query.name=my_query.name.replace("’","\'");

        console.log("b_name="+b_name_split+", my_query.name="+my_query.name);
        var x;
        for(x of b_name) {
            if(MTP.matches_names(x,my_query.name)) return false;
        }
    ...
    function init_Query()
    {
        ...
        my_query={name:document.querySelector("form h2").innerText.trim(),fields:{"website":""},done:{},submitted:false};
        my_query.city=table.rows[3].cells[1].innerText.trim();
        my_query.state=table.rows[4].cells[1].innerText.trim();
