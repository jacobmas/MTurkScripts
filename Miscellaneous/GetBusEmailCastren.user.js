...
(function() {
    ...
    var MTurk=new MTurkScript(40000,200,[],begin_script,"A237XQ0YKNKXRH",true);
    ...
    function query_response(response,resolve,reject,type) {
	    ...
	    if(b_context&&(parsed_context=MTP.parse_b_context(b_context))) {
            ...
            for(i=0; i < b_algo.length&&i<3; i++) {
                ...
                if(!MTurkScript.prototype.is_bad_url(b_url, bad_urls,-1)  && (b1_success=true)) break;
            }
    ...
    function add_to_sheet() {
        ...
        for(i=0;i<Gov.contact_list.length;i++) {
            ...
            if(Gov.contact_list[i].title && my_title_regex.test(Gov.contact_list[i].title)&&Gov.contact_list[i].email &&
             Gov.contact_list[i].email.length>0&&!MTP.is_bad_email(Gov.contact_list[i].email)) {
    ...
    var contact_response=function(doc,url,extra) {
        ...
        for(i=0; i < links.length; i++)
        {
          //  if(my_query.fields.email.length>0) break;
            ...
            if(/instagram\.com/.test(links[i].href) && my_query.done.insta===undefined && extension!=='info') {
            ...
            if(/facebook\.com/.test(links[i].href) && !my_query.done.fb && !my_query.found_fb && extension!=='info') {
...
//    else if(lim_one && /^[^\.]+\.[^\.]+(\.[a-z]{2}\.us)$/i) ret=ret;
    ...
    function init_Query() {
        ...
        my_query={name: wT.rows[0].cells[1].innerText, url: wT.rows[1].cells[1].innerText,
                  email_list:[],fb_url:wT.rows[4].cells[1].innerText,pending_gov_promises:0,
                  fields:{email:""}, done:{fb:false,url:false,fb_query:false,info_query:false,query:true},submitted: false};
       ...
       // if(!/https?:\/\/www\./.test(my_query.url)) my_query.url=my_query.url.replace(/(https?:\/\/)/,"$1www.");
        ...
        console.log("my_query="+JSON.stringify(my_query));
        var promise_list=[];
        if(my_query.fb_url && my_query.fb_url.length>0 && !is_bad_fb_url(my_query.fb_url)) {
            my_query.found_fb=true;
            my_query.done.fb_query=true;
            my_query.fb_about_url=my_query.fb_url.replace(/(facebook\.com\/[^\/]+).*$/,"$1")
                .replace(/facebook\.com\//,"facebook.com/pg/").replace(/\/$/,"")+"/about/?ref=page_internal";
...
/*            var search_str=my_query.name+" site:facebook.com";
            const queryPromise = new Promise((resolve, reject) => {
                ...
                query_search(search_str, resolve, reject, query_response,"fb_query");
            });
            queryPromise.then(fb_query_promise_then)
                .catch(function(val) {
                console.log("Failed at this fb_queryPromise " + val);
                my_query.done.fb_query=true;  my_query.done.fb=true;
                submit_if_done(); });*/
        }
