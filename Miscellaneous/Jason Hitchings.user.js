...
(function() {
    ...
    var MTurk=new MTurkScript(20000,750+(Math.random()*1000),[],begin_script,"A11USHJTJ9AOKY",true);
    ...
    function parse_sec(doc,url,resolve,reject) {
        var filed_re=/FILED AS OF DATE:\s*([\d]+)/;
        var cusip_re=/CUSIP (?:Number:|No\.).*\n+\s*([A-Z0-9]+\s?[A-Z0-9]*)/
        var months={"01":"January","02":"February","03":"March","04":"April","05":"May","06":"June","07":"July",
                    "08":"August","09":"September","10":"October","11":"November","12":"December"};
        let filed_match=doc.body.innerHTML.match(filed_re);
       // console.log("doc.body.innerHTML=",doc.body.innerHTML);
        console.log("filed_match=",filed_match);
        if(filed_match) {
            let my_date=filed_match[1];
            my_query.fields.filing_date=months[my_date.slice(4,6)]+" "+my_date.slice(6,8).replace(/^0+/,"")+", "+my_date.slice(0,4);
            add_to_sheet();
        }
        console.log(doc.body.innerText);
        var cusip_match=doc.body.innerText.match(cusip_re);
        if(cusip_match) {
            my_query.fields.cusip=cusip_match[1];
            add_to_sheet();
        }
        var table=doc.querySelectorAll("table[style$='collapse']");
        if(table.length<2) {
            reject("");
            return;
        }
	...
	console.log("my_query="+JSON.stringify(my_query));
        var search_str;
        var promise=MTP.create_promise(url,parse_sec,submit_if_done,function() { GM_setValue("returnHit",true); });
    }
